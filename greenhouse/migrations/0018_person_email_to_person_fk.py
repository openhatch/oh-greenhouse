...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for upload in orm.Uploads.objects.all():
            upload.person = orm.People.objects.get(email=upload.email_changer)
            upload.original_person = orm.People.objects.get(email=upload.original_email_changer)
            upload.save()

    def backwards(self, orm):
        for upload in orm.Uploads.objects.prefetch_related('person').prefetch_related('original_person'):
            upload.email_changer = upload.person.email
            upload.original_email_changer = upload.original_person.email
            upload.save()
